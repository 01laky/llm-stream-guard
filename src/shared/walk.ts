...
function collectFiles(path: string, files: string[], skipDirs: Set<string>): void {
	let st;
	try {
		st = statSync(path);
	} catch {
	...
	for (const name of readdirSync(path)) {
		...
		collectFiles(join(path, name), files, skipDirs);
	}
}

/** Recursively collect files under the given absolute paths. */
export function walkFiles(paths: string[], skipDirs: Set<string> = DEFAULT_SKIP_DIRS): string[] {
	const files: string[] = [];
	for (const p of paths) {
		collectFiles(p, files, skipDirs);
	}
	return files;
}
